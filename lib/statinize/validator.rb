module Statinize
  class Validator
    ...
    def validate
      attrs.each do |attr|
        attr.options.each do |validator, validator_value|
          ...
          validator_instance.instance_variable_set('@attr_value', instance.public_send(attr_name))
      ...
      # binding.pry
      raise ValidationError, errors.to_s if force? || should_raise?
    end
    ...
    def should_raise?
      @erroneous_attrs.intersect? forced_attributes.map(&:name)
    end
    ...
    def validators_exist?
      attrs
        ...
        .reject { |validator| validator == :force }
